...
function alterna_get_social_list($extra_name='',$topbar = false){
	...
	foreach($social_list as $social_item){
		...
		if(penguin_get_options_key('social-'.$social_item[0]) != '') {
			if(!$topbar) {
				$str .=  '<li><a title="'.$social_item[1].'" href="'.penguin_get_options_key('social-'.$social_item[0]).'" target="_blank" class="alterna-icon-'.$social_item[0].'"></a></li>';
			}else if($social_item[2] != 0){
				$str .=  ' <li class="social"><a href="'.penguin_get_options_key('social-'.$social_item[0]).'" target="_blank" ><i class="fa fa-'.($social_item[2] == 1 ? $social_item[0] : $social_item[3]).'"></i></a></li>';
			}
	...
	if ( comments_open() ) { ?>
		<div id="respond">
            ...
            <div class="form-submit">
                <button name="submit" type="submit" id="submit" class="btn btn-theme icon-btn"><span><i class="fa fa-comment-o"></i></span><?php _e('Post Comment', 'alterna'); ?></button>
 ...
 * @since alterna 7.0
 */
function alterna_get_blog_widget_post($type = "", $per_page = "", $orderby = "", $cat_in = "", $tag_in = "", $post__in = "", $post__not_in = "") {
 ...
 * @since alterna 7.0
 */
function alterna_get_portfolio_widget_post($type = "", $per_page = "", $orderby = "", $cat_in = "", $tag_in = "", $post__in = "", $post__not_in = "") {
	if($per_page == ""){
		$per_page = 4;
	}
